...
class Solution:
    def lastStoneWeight(self, stones: List[int]) -> int:
        max_heap = MaxHeap(nums=stones)

        while max_heap.length() > 1:
            print(max_heap.head_data)
            max_1 = max_heap.heappop()
            print(max_heap.head_data)
            max_2 = max_heap.heappop()
            print("*" * 10)

            if (max_2 - max_1) != 0:
                max_heap.heappush(max_1-max_2)

        return max_heap.peek() if max_heap.peek() else 0